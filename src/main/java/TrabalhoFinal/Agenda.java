...
public class Agenda {
    ...
    public void adicionarConsulta(Consulta consulta1) throws ValidacaoException {
        if (total >= 10) {
            throw new ValidacaoException("Agenda cheia. Limite atingido de 10 consultas");
        }
        consultas[total] = consulta1;
        total++;
    }

    public void cancelarConsulta(String nomeAnimal) throws ValidacaoException {
        if (nomeAnimal == null || nomeAnimal.trim().isEmpty()) {
            throw new ValidacaoException("Nome inválido. Tente novamente");
        }
